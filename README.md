...
<title>Генератор ASCII-арта из галереи</title>
<style>
* {
...
<div class="gallery-title">Выберите изображение из галереи:</div>
...
<div class="loading" id="loading">Преобразование изображения... Пожалуйста, подождите</div>
...
<div class="setting-item">
<div class="setting-label">
<span>Инвертировать цвета</span>
<span id="invertStatus">Нет</span>
</div>
<div class="setting-control">
<input type="checkbox" id="invertCheckbox">
<span>Включить для светлого фона</span>
</div>
</div>
</div>
</div>
<div class="controls">
<button id="convertBtn" class="btn btn-generate">✨ Сгенерировать ASCII-арт</button>
<button id="copyBtn" class="btn btn-copy">📋 Копировать</button>
<button id="downloadTextBtn" class="btn btn-download">💾 Скачать как текст</button>
<button id="downloadImageBtn" class="btn btn-download">🖼️ Скачать как изображение</button>
<button id="resetBtn" class="btn btn-reset">↺ Сбросить всё</button>
</div>
<div class="footer">
</div>
</div>
...
function loadImagesFromFolder() {
  ...
  imageNames.forEach(name => {
    ...
    img.onload = function() {
      ...
      galleryImg.src = name;
      galleryImg.alt = `Изображение ${displayedCount}`;
...
invertStatus.textContent = invertCheckbox.checked ? 'Да' : 'Нет';
...
function loadImage(src) {
  ...
  img.onload = function() {
    ...
    if (img.width > 4000 || img.height > 4000) {
      showError('Изображение слишком большое. Пожалуйста, используйте изображение меньше 4000x4000 пикселей.');
      return;
    }
  ...
  img.onerror = function() {
    showError('Не удалось загрузить изображение. Убедитесь, что файл существует в той же папке.');
...
function convertImageToAscii() {
  if (!imagePreview.src) {
    showError('Пожалуйста, выберите изображение из галереи');
    return;
  }
  ...
  if (!imagePreview.complete || imagePreview.naturalWidth === 0 || imagePreview.naturalHeight === 0) {
    showError('Изображение еще не загружено полностью. Подождите несколько секунд и попробуйте снова.');
    return;
  }
  ...
  setTimeout(() => {
    try {
      ...
      if (!Number.isFinite(width) || !Number.isFinite(height)) {
        throw new Error('Недопустимые размеры холста. Проверьте настройки ширины и высоты.');
      }
      
      // Set reasonable limits to prevent browser crashes (much higher than slider max)
      const MAX_CANVAS_SIZE = 10000;
      if (width > MAX_CANVAS_SIZE || height > MAX_CANVAS_SIZE) {
        throw new Error(`Размеры холста слишком большие (максимум ${MAX_CANVAS_SIZE}x${MAX_CANVAS_SIZE}). Уменьшите настройки ширины/высоты.`);
      }
      ...
      try {
        ...
        throw new Error("Ошибка доступа к данным изображения. Если вы открываете файл напрямую через файловую систему, запустите локальный веб-сервер для работы с изображениями. Или попробуйте другое изображение.");
      }
      ...
      if (data.length !== width * height * 4) {
        throw new Error("Поврежденные данные изображения. Попробуйте другое изображение.");
      }
      ...
      showError("Ошибка при преобразовании: " + (error.message || "Неизвестная ошибка. Попробуйте другое изображение или настройки."));
    } finally {
...
copyBtn.addEventListener('click', () => {
  if (!asciiOutput.textContent) {
    showError('Нет ASCII-арта для копирования. Пожалуйста, сначала сгенерируйте его.');
    return;
  }
  navigator.clipboard.writeText(asciiOutput.textContent).then(() => {
    ...
    showError('Не удалось скопировать. Пожалуйста, выделите и скопируйте текст вручную.');
  });
});
...
downloadTextBtn.addEventListener('click', () => {
  if (!asciiOutput.textContent) {
    showError('Нет ASCII-арта для скачивания. Пожалуйста, сначала сгенерируйте его.');
    return;
  }
...
downloadImageBtn.addEventListener('click', () => {
  ...
  if (lines.length === 0) {
    showError('Невозможно создать изображение из пустого ASCII-арта.');
    return;
  }
  ...
  if (canvasWidth <= 0 || canvasHeight <= 0) {
    showError('Невозможно создать изображение: недопустимые размеры.');
    return;
  }
  ...
  outputCanvas.toBlob(blob => {
    if (!blob) {
      showError('Ошибка создания изображения. Попробуйте уменьшить размер ASCII-арта.');
      return;
    }
